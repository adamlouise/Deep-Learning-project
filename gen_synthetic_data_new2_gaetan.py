...
if save_res:
    if SNR_dist == 'uniform':
        ...
        raise ValueError('Unknown SNR distribution %s' % SNR_dist)
    fname = os.path.join(save_dir, "training_data%s_%d_samples_lou_version6" %
                         (SNR_str, num_samples))
    scio.savemat(fname,
                 mdict,
                 format='5',
...
filename1 = os.path.join(save_dir, "DW_image_store_%s_%d__lou_version6" %
                          (SNR_str, num_samples))
filename2 = os.path.join(save_dir, "DW_noisy_store_%s_%d__lou_version6" %
                          (SNR_str, num_samples))
