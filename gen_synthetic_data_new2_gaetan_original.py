...
def get_csd_peaks(DWI_image_noisy, sch_mat_b0, num_fasc):
    ...
    peaks = peaks_from_model(model=csd_mod,
                             data=DW_image_noisy,
                             sphere=odf_sphere,
                             relative_peak_threshold=nu_min/nu_max,
...
if save_res:
    if SNR_dist == 'uniform':
        ...
        raise ValueError('Unknown SNR distribution %s' % SNR_dist)
    fname = os.path.join(save_dir, "training_data%s_%d_samples_lou_version1" %
                         (SNR_str, num_samples))
    scio.savemat(fname,
                 mdict,
                 format='5',
...
# filename1 = os.path.join(save_dir, "DW_image_store_%s_%d__lou_version1" %
#                          (SNR_str, num_samples))
# filename2 = os.path.join(save_dir, "DW_noisy_store_%s_%d__lou_version1" %
#                          (SNR_str, num_samples))
